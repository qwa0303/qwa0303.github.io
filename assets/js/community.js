...
const CommunitySystem = {
    ...
    updateStats: function() {
        ...
        try {
            ...
            posts.forEach(post => {
                totalCommentsCount += post.comments ? post.comments.length : 0;
            });
            
            console.log(`统计结果：用户${users.length}，帖子${posts.length}，评论${totalCommentsCount}`);
            
            // 更新页面显示
            if (totalUsersElem) {
                totalUsersElem.textContent = users.length;
                console.log('已更新用户数显示');
            }
            
            if (totalPostsElem) {
                totalPostsElem.textContent = posts.length;
                console.log('已更新帖子数显示');
            }
            
            if (totalCommentsElem) {
                totalCommentsElem.textContent = totalCommentsCount;
                console.log('已更新评论数显示');
            }
            
            // 更新侧边栏社区数据
            const statsItems = document.querySelectorAll('.stat-item strong');
            if (statsItems.length >= 3) {
                statsItems[0].textContent = users.length;
                statsItems[1].textContent = posts.length;
                statsItems[2].textContent = totalCommentsCount;
            }
